...
class WCGAC_Set_Coupon {
	...
	public function __construct() {	
		...
		if ( empty( self::$settings ) ) {
			...
			$settings['order_amount'] = get_option('wgac_order_amount') ? get_option('wgac_order_amount') : 100;
	...
	public static function add_coupon_to_myaccount(){
		...
		if( $new_status == "completed" ) {			
			...
			if(!$isCreatedBefore && $tot_order > self::$settings['order_amount'] ){
